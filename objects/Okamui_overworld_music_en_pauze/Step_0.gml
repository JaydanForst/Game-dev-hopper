...
if pause_btn != noone {
    ...
    if mouse_check_button_pressed(mb_left) && position_meeting(mouse_x, mouse_y, pause_btn) {
        ...
        if (!instance_exists(O_Pauze_menu_Naruto_1)) {
            instance_create_layer(
                ...
                "GUI",
                O_Pauze_menu_Naruto_1
            );
        }
    } else {

        with (O_Pauze_menu_Naruto_1) instance_destroy();
    }
  }
}
...
if paused {
    if bgm_instance != noone {
        audio_pause_sound(Obito_Ritual);
        bgm_instance = noone;
    }
} else {
    if bgm_instance == noone {
        bgm_instance = audio_resume_sound(Obito_Ritual);
    }
}